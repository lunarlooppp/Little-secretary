...
const server = new Server(
  {
    name: 'little-secretary-web-search',
    version: '0.1.0'
  },
  {
    capabilities: {
...
async function fetchPage(args) {
  ...
  const title = trimText($('title').first().text(), 180);
  const description = trimText($('meta[name="description"]').attr('content') ?? '', 300);
  const content = trimText($('body').text(), Math.max(1000, Math.min(20000, Number(args.max_chars ?? 8000))));
